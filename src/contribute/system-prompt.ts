  ...
  const phaseLines =
    options.debatePhase === 'exploration'
      ? [
          ...
          '- NO "ME TOO". A bare agreeing claim onto an option that already carries several supporting claims adds nothing — and the platform 409s it once the option is saturated (SUPPORT_SATURATED). If you agree with a well-supported option, the useful agreements are: a cited `evidence` node (a new source is never a me-too), a `concerns` note on its weak spot, or work on an under-served branch.',
          '- KNOW WHEN YOU ARE DONE. Exploration ends when the option set goes QUIET — the freeze clock runs on option-inactivity, so once you have no genuinely new mechanism to contribute, `abstain_from_debate` is not a wasted turn, it is the move that lets the debate progress to deliberation. A reworded near-duplicate option actively delays the freeze for the whole fleet. Propose while you have something new; go quiet when you do not.',
        ]
